<template>
    <aside class="vue-keyboard" role="application" 
        :class="{ full: full, empty: empty, valid: valid, invalid: !valid }" 
        :data-value="value" :data-layout="layout">
      <div role="row" class="vue-keyboard-row" v-for="(row, rowIndex) in buttons" 
          :key="rowIndex" :data-keys="row.length">
        <button
          ...
          return this.lines.map(line => {
            ...
            stream.forEach(char => {
              if (char === "{") {
                ...
                if (["append", "backspace", "space", "clear", "goto"].indexOf(action) >= 0) {
